...
function init() {
  ...
  // NOT re-added from main's b96630c: a ['income','premium'] focus->select()
  // listener. initMoneyInputs() above already binds select-on-entry to every
  // input[data-money] — which is these two, plus #regRate — and does it properly:
  // a bare focus handler is undone by the tap's own caret placement on mouseup,
  // which is exactly the case money-input.js's pointerdown/click re-select
  // exists to cover. A second listener here would be a weaker duplicate.
  // The form-level one-shot clearExampleNote is likewise gone on purpose: the
  // per-field noteFieldTouched bookkeeping above replaces it and only drops the
  // example label once every number the answer is built from is the visitor's.
  render();
  announceReady = true;
}
